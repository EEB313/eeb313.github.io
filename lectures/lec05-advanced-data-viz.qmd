...
It is possible to specify exactly which colors[^lec05-advanced-data-viz-1] to use and to change the thickness of the lines to make the them easier to distinguish.

[^lec05-advanced-data-viz-1]: There are so many colors to chose from in R. Check out the [R Color](http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf) doc to find something that brings you joy.
...
Lastly, let's change the x labels so that they don't overlap, split the subplot titles into multiple lines if they're too long, and remove the grey background to increase contrast with the lines. To customize the non-data components of the plot, we will pass some *theme* statements[^lec05-advanced-data-viz-2] to `ggplot`.

[^lec05-advanced-data-viz-2]: The amount of control over various plot elements in `ggplot` is truly astonishing. Check out the complete list of themes [here](https://ggplot2.tidyverse.org/reference/theme.html). Have fun!
