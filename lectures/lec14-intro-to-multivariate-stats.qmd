...
[**Multivariate statistics**]{.underline} are a family of statistical tools used to to handle situations where the predictor and/or the response variables come in sets, allowing us to distill through some of that correlated (and sometimes redundant) information and to analyze these variables simultaneously. These tools are also useful for identifying substructure or groupings of data.

Common biological questions that multivariate statistics can be used to answer:

-   Do sites with similar disturbance regimes (history of fire, nutrient fertilization, soil chemicals) host similar biological communities? (could use a [PCA](https://uw.pressbooks.pub/appliedmultivariatestatistics/chapter/pca/), [NMDS](https://uw.pressbooks.pub/appliedmultivariatestatistics/chapter/nmds/), [PCoA](https://uw.pressbooks.pub/appliedmultivariatestatistics/chapter/pcoa/))

-   What is the effect of a set of environmental variables (temperature, salinity, current strength) on the composition of fish communities? (could use a [RDA](https://r.qcbs.ca/workshop10/book-en/redundancy-analysis.html))

-   Do communities in different habitats contain a different set of functional traits? (could use a [PCA](https://uw.pressbooks.pub/appliedmultivariatestatistics/chapter/pca/), [NMDS](https://uw.pressbooks.pub/appliedmultivariatestatistics/chapter/nmds/), [PCoA](https://uw.pressbooks.pub/appliedmultivariatestatistics/chapter/pcoa/))

## Bird morphology data

Today we will be introducing the world of multivariate statistics by working with a published data set from Pigot et al, "[Macroevolutionary convergence connects morphological form to ecological function in birds]{.underline}*", Nature Ecology & Evolution*, 2020 ([here](https://www.nature.com/articles/s41559-019-1070-4)). In this paper, the authors ask if **morphology is linked to ecological function in birds**. We might expect this to be true if species have evolved traits (in this case morphological traits) to optimize their fitness in a specific abiotic environment. In this way, traits can be used to link species to theories of community assembly, niche partitioning, and local adaptation. A nice general interest summary of the paper is provided [here](https://www.nature.com/articles/s41559-019-1078-9).

![](figures/pigot_figED1.png)

![](figures/pigot_fig1.png)
...
A)  Do birds that are more closely related (eg. within order) tend to have similar trait values and occupy the same regions of morphospace (this would be an indication of phylogenetic inertia)?
...
library(tidyverse)
library(rgl) #package to make interactive 3D plots
library(ggfortify) #package to make plots from output of PCA and other stats packages
...
```{r read in data}
# read in bird data
birds <- read.csv("data/bird_traits.csv") # if you've already downloaded it
# birds <- read.csv("https://eeb313.github.io/lectures/data/bird_traits.csv") # read directly from class website
...
### Two-dimensional (2D) trait space
...
### Three-dimensional (3D) trait space
...
```{r 3D plot with example species, eval=FALSE}
plot3d(birds$Mass, birds$Beak.Width, birds$Wing.Length, col=alpha("grey80", .01))
...
plot3d(birds$Mass, birds$Beak.Width, birds$Wing.Length, col=alpha("black", .1), size=1)
...
```{r PC2, PC3, eval=FALSE}
options(rgl.useNULL = FALSE)
dirVector2 <- xyz_pca$rotation[, 2]  # PC2
dirVector3 <- xyz_pca$rotation[, 3]  # PC3
rgl::open3d()
plot3d(birds$Mass, birds$Beak.Width, birds$Wing.Length, col=alpha("black", .1), size=1)
abclines3d(mean_xyz, a = t(dirVector1), col="darkorange", lwd=2) # mean + t * direction_vector of PC1
abclines3d(mean_xyz, a = t(dirVector2), col="darkblue", lwd=2) # mean + t * direction_vector of PC2
abclines3d(mean_xyz, a = t(dirVector3), col="darkgreen", lwd=2) # mean + t * direction_vector of PC3
```
...
### Applications of PCA to biological questions

Recall the way we started this lecture, we want to be able to answer if:

1.  Do birds that are more closely related (eg. within order) tend to have similar trait values ie occupy the same regions of morphospace (indication of phylogenetic inertia)?

2.  Do birds that occupy similar trophic niches have similar traits, as we might expect from trait-based theories of ecology and convergent evolution?
...
## Applications of PCA in the biological literature
...
### Example from Pigot et al, "**Macroevolutionary convergence connects morphological form to ecological function in birds", Nature Ecology & Evolution, 2020**

*What can we conclude from this Figure 6?*

![](figures/pigot_fig6.png){width="642"}
...
![](figures/microplastics_fig7.png)

### Population genetics example

The authors in this paper, "[Getting genetic ancestry right for science and society](10.1126/science.abm7530)", argue that in a human genetic context, PCAs can do more harm than good when artificial categories are applied to continuous data with no discrete breaks. For example, they show a PCA plot of human genetic ancestry–defined by migration and high mixing–and attempt to impost discrete clusters of continental origin on the PC space.
