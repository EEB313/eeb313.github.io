...
[^lec09-linear-models-1]: Whitman, K., Starfield, A., Quadling, H. & Packer, C. *Sustainable trophy hunting of African lions*. **Nature** 428, 175–178 (2004). <https://doi.org/10.1038/nature02395>
...
$$ L(\boldsymbol{x}_i,\boldsymbol{y}_i|\beta_0,\beta_1,\sigma^2) = Normal(\mu = \beta_0 + \beta_1 x,\sigma^2) =\frac{1}{\sqrt{2 \pi \sigma^2}} e^{-(y-\mu)^2/2\sigma^2}.$$
...
*Hint: Don't get confused by the naming of the argument `x` in `dnorm` ... `x` is the value you want to calculate the probability (aka likelihood) of. In our case, we are calling this `Y`.*
...
```{r}
B0 <- lionFitOutputSummary$coefficients["(Intercept)","Estimate"]
B1 <- lionFitOutputSummary$coefficients["proportionBlack","Estimate"]

# predict lion age if 50% nose black
B0 + B1*0.5
```
...
2.  Is there significant evidence that age varies with the proportion of the nose that is black? Describe the results of the hypothesis test in careful statistical language.
    ...
    *Answer: Yes, there is significant evidence of a linear relationship between the proportion of the nose that is black and age among males lines, and we can reject the null hypothesis of no relationships (i.e., that the slope is zero, p \< 1e-7)*
...
*Answer: just looking at the graph, it seems like at a proportion black as low as 0.1 and as high as 0.8, an age of 6 years is within the 95% prediction interval. These values correspond to the mean proportions for ages 4 and 9, respectively.*
...
First, let's quickly look at our `mass` values. Remember this study includes a huge range of mammals, all the way from the tiny 3g Pygmy shrew to the 12 metric tonne Baird's beaked whale! If we try to visualize the distribution of masses on a regular scale, we see that the masses are very non-evenly distributed and have a massive range.

```{r}
ggplot(mammal_long, aes(x = mass))+ geom_histogram()
```

This is the perfect situation to apply a log transform. We'll use log base 10, since it lends to an easy interpretation (if log10(mass) = 3, the mass is 10\^3 g = 1000 g = 1 kg)

```{r}
ggplot(mammal_long, aes(x = log10(mass)))+ geom_histogram()
```
...
Why is it assumed to be common knowledge that males are larger than females, if the data seems to show no evidence of this? The authors hypothesized that this might be due to most research focusing on larger, more "charismatic" animals, like primates, felines and canines, undulates, livestock, etc. Maybe there is sexual dimorphism in some groups of mammals but not others. To examine this, let's extend our analysis of the impact of sex on body mass by trying to see if there is a different relationship depending on the Order the species belongs to. To do this, we'll regress on both sex and Order, and their interaction.

First, we'll filter for Orders that have at least 10 species with both male and female data, since we expect it to be difficult to identify effects otherwise. Note that we don't need to do this : regression analyses already takes into account uncertainty due to low samples and be very unlikely to reject a null hypothesis - but it doing so will simplify our output and interpretation
