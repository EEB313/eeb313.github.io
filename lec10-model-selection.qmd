...
> ### Learning objectives
>
> - Understand the difference between liklihood estimation/inference and model _selection_
> - Develop familarity with common model selection approaches
> - Understand intuion for and the use of common informatic theoretic model selection criteria
> - Perform model selection on the RIKZ data from last class
> 
> ### Lesson outline
> 
> - Brief intro to model selection (20 mins)
> - Understanding informatic theoretic criteria (20 mins)
...
Model selection for mixed models is even more complicated, due to the addition of random effects. A procedure that is commonly used to preform selection of LMMs is as follows:
   ...
   2. Optimize the random-effect structure by comparing models with the same saturated fixed effects structure but differing random effect structures. These models should be fitted using Restricted Maximum Likelihood (i.e., `REML = TRUE`), since variance component estimation will be biased otherwise. The _optimal_ random effect structure is the one that provides the lowest AIC. Do **not** remove random effects if they are included to account for non-independence in your data. Skip this step if random effects must be included given the hierarchical structure (i.e., nestedness) in the data.
...
Finally, let's re-fit the best-fitting model:
...
We see that increasing both NAP and Exposure results in a significant decrease in species richness ($p<0.05$). There is also a marginally interaction between NAP and Exposure which we won't read too much into ($p=0.041$!). Finally, while Beach is included in our model as a random effect, notice how little variation is attributed to differences between beaches relative to the model we ran in lecture 7 (~45% of residual variance vs. approx. 0). The only difference is that our current model includes Exposure as a fixed effect. This suggests that much of the variation between beaches in lecture 7 was likely attributable to differences in exposure, which is now being captured by the fixed effects.

What does that interaction between NAP and exposure really mean? Let's visualize our fitted model.

```{r}
rikz_data <- rikz_data %>%
  mutate(fit_full= predict(mixed_model_IntOnly_Full2))

ggplot(rikz_data, aes(x = NAP, y = Richness, colour = Exposure)) +
  geom_line(aes(y = fit_full, group = Exposure)) + 
  geom_point()
```
From the plot, we can see that there is a significant negative effect of NAP on Richness (the slopes are negative while holding Exposure constant), and there is a significant negative effect of Exposure on Richness (at higher exposures, the species richness is lower while holding NAP constant). There is also a significant positive effect of the interaction between NAP and Richness. That is, the way NAP affects Richness *depends* on Exposure, and the way Exposure affects Richness *depends* on NAP. Think of this as a difference in slopes. The trajectory of the negative slope becomes *less negative*, or positive, with one unit simultaneous increase in both Exposure and NAP. If we extrapolate to higher levels of NAP or Exposure, these lines may intersect. In other words, the difference in Richness is reinforced at a lower NAP and a lower Exposure.
